locals {
  virtual_network_name = (var.use_product_name_in_name != null ? "${var.names.product_name}-${var.names.subscription_type}-${var.names.location}-vnet" : "${var.names.product_group}-${var.names.subscription_type}-${var.names.location}-vnet")
