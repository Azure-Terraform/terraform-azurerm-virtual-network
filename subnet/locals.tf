locals {
  ...
  network_security_group_name = (var.security_group_perfix != null ? "${var.security_group_perfix}-${var.subnet_type}-security-group" : "${var.names.resource_group_type}-${var.names.product_name}-${var.subnet_type}-security-group")
