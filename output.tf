...
output "aks" {
  ...
  value = { for aks_id, info in local.aks_info :
    aks_id => {
      subnet = {
        name                        = try(module.aks_subnet["aks-${aks_id}"].subnet.name, module.aks_subnet[aks_id].subnet.name)
        id                          = try(module.aks_subnet["aks-${aks_id}"].subnet.id, module.aks_subnet[aks_id].subnet.id)
        resource_group_name         = try(module.aks_subnet["aks-${aks_id}"].subnet.resource_group_name, module.aks_subnet[aks_id].subnet.resource_group_name)
        address_prefixes            = try(module.aks_subnet["aks-${aks_id}"].subnet.address_prefixes, module.aks_subnet[aks_id].subnet.address_prefixes)
        service_endpoints           = try(module.aks_subnet["aks-${aks_id}"].subnet.service_endpoints, module.aks_subnet[aks_id].subnet.service_endpoints)
        network_security_group_id   = try(module.aks_subnet["aks-${aks_id}"].network_security_group_id, module.aks_subnet[aks_id].network_security_group_id)
        network_security_group_name = try(module.aks_subnet["aks-${aks_id}"].network_security_group_name, module.aks_subnet[aks_id].network_security_group_name)
        virtual_network_name        = azurerm_virtual_network.vnet.name
        virtual_network_id          = azurerm_virtual_network.vnet.id
